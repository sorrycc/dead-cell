...
// ── STARTING-STATS CONTRACT (Decision 57/60) ── the single object the meta fold produces + the Player
// reads. Exported so MetaState (BASE_PLAYER_STATS) and the Player consume the SAME shape (DRY). Each
// upgrade's `apply` takes one of these and returns a NEW one (never mutates — referential safety, AC55).
...
// ── UPGRADE ROW (Decision 57) ── one self-contained meta-upgrade entry. Exported so consumers
// (MetaState.buy / applyUpgrades, the Hub render) type the rows they read from UPGRADES/UPGRADES_BY_ID.
export interface Upgrade {
  ...
  maxLevel: number
  costs: number[]
  apply: (stats: PlayerStats, level: number) => PlayerStats
}
...
const START_WEAPON_BY_LEVEL: string[] = ['sword', 'hammer', 'bow'] // index = owned level (0 = default sword).
...
export const UPGRADES_BY_ID: Record<string, Upgrade> = Object.fromEntries(UPGRADES.map((u) => [u.id, u]))
