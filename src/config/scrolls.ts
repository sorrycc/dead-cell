...
// The subset of RunState fields a scroll's apply() reads/mutates IN PLACE (the run-only economy +
// the two existing run/player resources two scrolls bump directly). Consumer modules type their
// live RunState against this so the engine's hit/dodge/flask sites read the same shape.
export interface ScrollRunState {
...
export interface ScrollSpec {
  id: string
  name: string
  apply: (run: ScrollRunState) => void
}

export const SCROLLS: ScrollSpec[] = [
