...
export class ParticlePool {
  ...
  spawnSparks(x: number, y: number, { count = 8, color = 0xffe066, speed = 260 }: { count?: number; color?: number; speed?: number } = {}) {
  ...
  spawnNumber(x: number, y: number, value: number | string, { color = '#ffffff', scale = 1 }: { color?: string; scale?: number } = {}) {
