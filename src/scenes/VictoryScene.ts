...
const DEFAULT_SUMMARY: RunSummary = { depthReached: 0, biomeName: '—', timeMs: 0, kills: 0, cellsBanked: 0, runSeed: 0, completed: true }

// Format a run seed as the shareable hex run id (Decision 71) — what you type into the Hub to replay.
const formatSeed = (s: number) => `0x${(s >>> 0).toString(16)}`
...
export class VictoryScene extends Phaser.Scene {
  ...
  create(data: Partial<RunSummary>) {
