...
// Combatant shape both directions share: body CENTER x + facing ∈ {-1,+1}.
export interface Combatant {
  cx: number
  facing: number
}

// One row from a weapon's swing table — resolveHit reads `damage` + `knockback`.
export interface SwingSpec {
  damage: number
  knockback: number
}

// Per-call gates (see the comment below for the load-bearing semantics of each flag).
export interface ResolveHitOpts {
  allowBackstab?: boolean
  damageMult?: number
}

// Plain decision the caller applies in its own onHit.
