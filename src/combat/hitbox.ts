...
// The minimal attacker shape swingRect reads — body CENTER (cx/cy) + facing ∈ {-1,+1}. Both the
// Player and the Enemy already expose this, so the same function serves player→enemy AND enemy→player (DRY).
export interface SwingAttacker {
  cx: number
  cy: number
  facing: number
}

// The world-space AABB swingRect returns — CENTER (x,y) + size (w,h). HitboxPool.acquire stamps these onto a pooled body.
...
export function swingRect(attacker: SwingAttacker, swing: SwingRow): SwingRect {
  const w = swing.reach
