...
import type { SwingAttacker, SwingRect } from './hitbox.js'
import type { SwingRow } from '../config/weapons.js'

// Per-hitbox combat context mutated on acquire (never re-allocated → no per-hit GC). Stamped onto
// the pooled Rectangle as `.hb`; the scene's overlap callbacks read ownerTag/ownerId/hitSet/swing.
interface HitboxContext {
  active: boolean
  ownerId: string | null
  ownerTag?: string
  swing: SwingRow | null
...
// A pooled Rectangle carries its combat context on `.hb` (a dynamic prop Phaser doesn't know about).
type HitboxRect = Phaser.GameObjects.Rectangle & { hb: HitboxContext }
...
export class HitboxPool {
  ...
  constructor(scene: Phaser.Scene, ownerTag: string, size = 4) {
    this.scene = scene
    ...
    for (let i = 0; i < size; i++) {
      ...
      const rect = scene.add.rectangle(0, 0, 10, 10, HITBOX_COLOR).setAlpha(0) as HitboxRect
      this.group.add(rect)
  ...
  acquire(attacker: SwingAttacker, swing: SwingRow, ownerId: string): HitboxRect | null {
    const rect = this._items.find((r) => !r.hb.active)
    if (!rect) return null

    const aabb: SwingRect = swingRect(attacker, swing)
    const body = rect.body as Phaser.Physics.Arcade.Body
