...
import type { LevelDescription, Platform } from './LevelGenerator.js'
import type { BiomeColors } from '../config/biomes.js'

// The SUBSET of BiomeColors TileMap actually reads (the fallback below carries only these four).
type TileColors = Pick<BiomeColors, 'solid' | 'oneWay' | 'hazard' | 'bg'>
...
export class TileMap {
  ...
  hazardBodies!: Phaser.Physics.Arcade.StaticGroup
  private _bg!: Phaser.GameObjects.Rectangle
  private _hazardBodiesEnabled!: boolean
  ...
  constructor(scene: Phaser.Scene, desc: LevelDescription) {
    this.scene = scene
    this.desc = desc
    const colors: TileColors =
      (scene as { biome?: { colors?: BiomeColors } }).biome?.colors ?? desc.colors ?? FALLBACK_COLORS
    ...
    for (let row = 0; row < desc.rows; row++) {
      for (let col = 0; col < desc.cols; col++) {
        ...
        const rect = scene.add.rectangle(
          ...
          TILE_SIZE * 0.5,
          colors.hazard,
        ) as Phaser.GameObjects.Rectangle & { tileCol: number; tileRow: number }
  ...
  _addRun(group: Phaser.Physics.Arcade.StaticGroup, run: Platform, color: number) {
...
const FALLBACK_COLORS: TileColors = {
  solid: 0x3a4658,
  oneWay: 0xb9770e,
  hazard: 0xc0392b,
  bg: 0x10141c,
