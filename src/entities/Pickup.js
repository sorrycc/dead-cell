...
export class PickupPool {
  ...
  constructor(scene, size = 32) {
    ...
    for (let i = 0; i < size; i++) {
      const rect = scene.add.rectangle(0, 0, PICKUP_SIZE, PICKUP_SIZE, 0xffffff).setVisible(false).setDepth(40)
      this.group.add(rect)
  ...
  tick() {
    for (const rect of this._items) {
      ...
      rect.setPosition(rect.body.center.x, rect.body.center.y)
    }
  }
