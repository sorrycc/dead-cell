...
// The pooled hitbox rect HitboxPool.acquire hands back (carries its combat context on `.hb`).
type StrikeRect = ReturnType<HitboxPool['acquire']>

// The per-frame tick context GameScene passes — { player, effects }. Kept local: a structural shape over
// the live player + the FX facade (Phaser-adjacent dynamic surfaces consumed via targeted access below).
interface BossUpdateCtx {
  player: { body: Phaser.Physics.Arcade.Body }
  effects?: any
}

// GameScene exposes a couple of boss-only dynamic members (the FX facade + the summon hook) on top of the
// base Phaser.Scene; a targeted local widening keeps those reads honest without re-describing the scene.
type BossScene = Phaser.Scene & {
  effects?: { hit(x: number, y: number, result: { damage: number; isBackstab: boolean }): void }
...
export class Boss {
  ...
  minX: number
  maxX: number
  collider!: Phaser.GameObjects.Rectangle & { enemyRef?: Boss }
  ...
  dead: boolean
  removed: boolean
  onBossDeath: (() => void) | null
  onDeath: (() => void) | null
  stateTimer: number
  attackCooldownTimer: number
  contactCooldownTimer: number
  hurtIframeTimer: number
  phaseInvulnTimer: number
  deathTimer: number
  _patternIndex: number
  _currentAttack: BossAttackSpec | null
  ...
  constructor(scene: BossScene, x: number, y: number, spec: BossSpec, hitboxPool: HitboxPool, projectilePool: ProjectilePool, { minX = x - 400, maxX = x + 400 }: { minX?: number; maxX?: number } = {}) {
    this.scene = scene
    this.spec = spec
  ...
  applyStatus(spec: Parameters<typeof applyStatus>[1]) {
  ...
  update(dt: number, ctx: BossUpdateCtx) {
  ...
  _tickChoose(dt: number, ctx: BossUpdateCtx) {
  ...
  _tickTelegraph(dt: number, ctx: BossUpdateCtx) {
    this.body.setVelocityX(0)
  ...
  _tickStrike(dt: number, ctx: BossUpdateCtx) {
  ...
  _tickRecover(dt: number, _ctx?: BossUpdateCtx) {
    this.body.setVelocityX(0)
    this.stateTimer -= dt
    if (this.stateTimer <= 0) {
  ...
  _tickStatus(dt: number, ctx: BossUpdateCtx): boolean {
  ...
  _fireAttack(atk: BossAttackSpec, ctx: BossUpdateCtx) {
    if (atk.kind === 'slam') {
      const attacker = { cx: this.body.center.x, cy: this.body.center.y, facing: this.facing }
      this.strikeRect = this.hitboxPool.acquire(attacker, atk.swing as any, this.id)
  ...
  _fireVolley(atk: BossAttackSpec, ctx: BossUpdateCtx) {
  ...
  _fireSummon(atk: BossAttackSpec) {
  ...
  _showTelegraph(atk: BossAttackSpec, ctx: BossUpdateCtx) {
  ...
  _updateTelegraphFx(atk: BossAttackSpec | null, ctx: BossUpdateCtx) {
    if (!atk) return
    const cx = this.body.center.x
    const cy = this.body.center.y
    let w = 60
  ...
  _stepTowardPlayer(ctx: BossUpdateCtx, speed: number) {
    const cx = this.body.center.x
