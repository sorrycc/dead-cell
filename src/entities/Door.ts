...
import type { LevelPoint } from '../world/LevelGenerator.js'
...
export class Door {
  ...
  constructor(scene: Phaser.Scene, exit: LevelPoint, color: number, onExit: () => void) {
    this.scene = scene
    this.onExit = onExit
    ...
    ;(this.rect as any).doorRef = this // back-ref so the overlap callback resolves the Door from its body.
